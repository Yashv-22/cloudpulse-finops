...
import { getEC2Client, getS3Client, getEnabledRegions } from "@/lib/aws/client";
...
export async function getUnattachedEBS() {
  ...
  if (isDemo) {
    return {
      data: [
        { id: "vol-0987654321fedcba1", type: "EBS (gp2)", size: 100, monthlyLeakage: 950, region: "ap-south-1" },
        { id: "vol-0987654321fedcba2", type: "EBS (gp2)", size: 100, monthlyLeakage: 950, region: "us-east-1" },
      ]
    };
  }
    ...
    await Promise.all(
      regions.map(async (region) => {
        try {
          ...
          // ignore individual region
        }
      })
    );
    
    return { data: allUnattached };
  } catch (error: any) {
    return { error: { service: "Storage (EBS)", message: "Permissions Required" } };
  }
}
...
export async function getUnoptimizedS3() {
  ...
  if (isDemo) {
    return {
      data: [
        { id: "customer-data-backups-2023", type: "S3 Bucket", monthlyLeakage: 1850, region: "Global" },
        { id: "app-logs-archive-prod", type: "S3 Bucket", monthlyLeakage: 850, region: "Global" },
      ]
    };
  }
  ...
  try {
    ...
    return { error: { service: "Storage (S3)", message: "Permissions Required" } };
  }
}
