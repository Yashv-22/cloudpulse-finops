...
import { Cloud, Bell, Shield, Plus, Check, Unlink, User, Lock } from "lucide-react";
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <header className="mb-2">
        ...
        <p className="text-[var(--text-muted)]">Manage integrations, alerts, and platform automation.</p>
      </header>

      {/* Profile */}
      <GlassCard className="p-6">
        <div className="flex items-center gap-3 mb-6">
          <User className="w-5 h-5 text-[var(--brand)]" strokeWidth={1.5} />
          <h2 className="text-lg font-semibold">Profile</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-[var(--text-muted)] mb-1 block">Name</label>
            <p className="text-sm text-[var(--text-primary)]">{user ? `${user.first_name} ${user.last_name}` : "Admin User"}</p>
          </div>
          <div>
            <label className="text-xs text-[var(--text-muted)] mb-1 block">Email</label>
            <p className="text-sm text-[var(--text-primary)]">{user?.email || "admin@cloudpulse.com"}</p>
          </div>
          <div>
            <label className="text-xs text-[var(--text-muted)] mb-1 block">Role</label>
            <p className="text-sm text-[var(--text-primary)] capitalize">{user?.role || "admin"}</p>
          </div>
        </div>
      </GlassCard>
      ...
      <GlassCard className="p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div>
            <label className="text-xs text-[var(--text-muted)] mb-1 block">Current Password</label>
      ...
      <GlassCard className="p-6">
        <div className="flex items-center gap-3 mb-6">
          <Cloud className="w-5 h-5 text-[var(--brand)]" strokeWidth={1.5} />
          <h2 className="text-lg font-semibold">AWS Integration</h2>
        </div>
        <div className="space-y-3">
          {(data?.aws_accounts || []).map((acc: any) => (
            <div key={acc.id} className="flex justify-between items-center p-4 bg-[var(--bg-panel)] rounded-lg border border-[var(--border-subtle)]">
              <div>
                <p className="font-medium text-[var(--text-primary)]">{acc.name}</p>
                <p className="text-sm text-[var(--text-muted)]">ID: {acc.id} ({acc.region})</p>
              </div>
          ...
          {showConnect && (
            <div className="p-4 bg-[var(--bg-panel)] rounded-lg border border-[var(--border-subtle)] space-y-3">
      ...
      <GlassCard className="p-6">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-[var(--text-primary)]">Zero-Touch FinOps</p>
              <p className="text-sm text-[var(--text-muted)]">Automatically apply safe cost-saving recommendations without manual approval.</p>
            </div>
            <ToggleSwitch
          ...
          <div className="flex items-center justify-between border-t border-[var(--border-subtle)] pt-6">
            <div>
              <p className="font-medium text-[var(--text-primary)]">Auto-Remediate Critical Security Risks</p>
              <p className="text-sm text-[var(--text-muted)]">Instantly execute AI-generated Terraform to patch critical vulnerabilities.</p>
            </div>
            <ToggleSwitch
      ...
      <GlassCard className="p-6">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-[var(--text-primary)]">Cost Anomaly Detection</p>
              <p className="text-sm text-[var(--text-muted)]">Send alerts for abnormal spending spikes.</p>
            </div>
            <ToggleSwitch
          ...
          <div className="flex items-center justify-between border-t border-[var(--border-subtle)] pt-6">
            <div>
              ...
              <p className="text-sm text-[var(--text-muted)]">Receive daily digest and critical alert emails.</p>
            </div>
            <ToggleSwitch
          ...
          <div className="flex items-center justify-between border-t border-[var(--border-subtle)] pt-6">
            <div>
              <p className="font-medium text-[var(--text-primary)]">Slack Integration</p>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                data?.alerts?.slack_connected
                  ...
                  : "bg-[#4A154B]/80 hover:bg-[#4A154B] border border-[#4A154B] text-[var(--text-primary)]"
              }`}
