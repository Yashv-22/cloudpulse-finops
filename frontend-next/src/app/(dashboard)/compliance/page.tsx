  ...
  return (
    <div className="space-y-6">
      <header className="mb-2">
        ...
        <p className="text-zinc-500">Real-time security posture and compliance audit results.</p>
      </header>
          ...
          {loading ? (
            ...
            <>
              <div className="relative w-32 h-32 flex items-center justify-center mb-4">
                ...
                <span className="absolute text-3xl font-bold text-white">{data?.compliance_score || 0}%</span>
              </div>
            ...
            <div className="space-y-2">
              {(data?.passed_checks || []).map((c, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-green-950/20 border border-green-500/10">
                  ...
                  <div>
                    <p className="text-sm font-medium text-zinc-200">{c.check}</p>
                    <p className="text-xs text-zinc-500">{c.resource_type}: {c.resource_id}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {(data?.failed_checks || []).map((c, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-red-950/20 border border-red-500/10">
                  ...
                  <div>
                    <p className="text-sm font-medium text-zinc-200">{c.issue}</p>
                    <p className="text-xs text-zinc-500">{c.resource_type}: {c.resource_id}</p>
